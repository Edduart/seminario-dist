...
class CreateTestDto {
    ...
    static create(props) {
        ...
        if (isNaN(Number(academic_term_id)) ||
        ...
        for (const test of tests) {
            ...
            if (!test.maximum_score) {
                ...
                if (isNaN(Number(test.maximum_score)) ||
                    typeof test.maximum_score !== "number" ||
                    test.maximum_score < 1 ||
                    test.maximum_score > 100)
                    validationErrors.push({
                        field: "maximum_score",
                        message: "maximum_score must be a number more than 1 and less than 100",
                    });
                break;
            }
        }
        ...
        const testScoreToUpperCase = tests.map((test) => ({
